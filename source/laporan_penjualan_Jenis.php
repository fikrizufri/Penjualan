...
$kodeJenis = isset($_POST['cmbJenis']) ? $_POST['cmbJenis'] : 'ALL';
	
if($_GET) {
	if (isset($_POST['btnCetak'])) {
			echo "<script>";
			echo "window.open('source/cetak_penjualan_Jenis.php?kodeJenis=$kodeJenis&tglStart=$tglStart&tglEnd=$tglEnd', width=330,height=330,left=100, top=25)";
			echo "</script>";
	}
	
	if (isset($_POST['btnTampil'])) {
		if (trim($_POST['cmbJenis'])=="ALL") {
			$filterSQL 	= "";
		}
		else {
...
?>
<div class='hero-unit'><h2 align="left">Laporan Penjualan Jenis Barang</h2>
</div>
<form action="?page=LaporanPenjualanJenis" method="post" name="form1" target="_self">
  <table width="500" border="0"  class="table table-condensed"><tr>
      ...
      <td><select name="cmbJenis">
        ...
        <?php
	  $mySql = "SELECT * FROM Jenis ORDER BY kd_Jenis";
	  $myQry = mysql_query($mySql, $koneksidb) or die ("Gagal Query".mysql_error());
	  while ($kolomData = mysql_fetch_array($myQry)) {
	  	if ($kodeJenis == $kolomData['kd_Jenis']) {
			$cek = " selected";
		} else { $cek=""; }
	  	echo "<option value='$kolomData[kd_Jenis]' $cek>$kolomData[nm_Jenis]</option>";
	  }
	  $mySql ="";
	  ?>
      </select></td>
...
<table class="table table-condensed" width="800" border="0" cellspacing="1" cellpadding="2"><tr>
    <td width="46" align="center" ><b>No</b></td>
    <td width="98" align="center" ><b>Kode</b></td>
    <td width="528" ><b>Nama Barang</b></td>
    <td width="107" align="center" ><b>Qty Terjual</b></td>
  </tr>
	<?php
