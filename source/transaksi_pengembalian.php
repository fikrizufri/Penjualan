...
if(isset($_POST['btnTambah'])){
	...
	if ($cek2Row['stok_opname'] < $txtJumlah) {
		$pesanError[] = "<b>Stok Opname</b> untuk kode <b>$txtKodeBarcode</b> adalah <b> $cek2Row[stok_opname]</b>, jadi tidak dapat dipengembalian!";
	}
	...
	if (count($pesanError)>=1 ){
		...
		$cekSql ="SELECT * FROM tmp_pengembalian As tmp, barang 
		...
		if (mysql_num_rows($cekQry) >= 1) {
			...
			$tmpSql = "UPDATE tmp_pengembalian SET jumlah=jumlah + $txtJumlah 
			...
			if ($myQty >= 1) {
				...
				$tmpSql 	= "INSERT INTO tmp_pengembalian (kd_supplier, kd_barang, jumlah, keterangan, kd_user) 
...
if(isset($_POST['btnSave'])){
	...
	$tmpSql ="SELECT COUNT(*) As qty FROM tmp_pengembalian WHERE  kd_user='".$_SESSION['SES_LOGIN']."'";
	...
	$tmp2Sql ="SELECT supplier.* FROM tmp_pengembalian, supplier WHERE supplier.kd_supplier=tmp_pengembalian.kd_supplier 
	...
	if (count($pesanError)>=1 ){
		...
		$noNota	= buatKode("pengembalian", "BB");
		$mySql	= "INSERT INTO pengembalian (no_pengembalian, tgl_pengembalian, kd_supplier, kd_user) 
					VALUES ('$noNota', 
		...
		if($myQry){
			...
			$tmp3Sql ="SELECT * FROM tmp_pengembalian WHERE kd_user='".$_SESSION['SES_LOGIN']."'";
			$tmp3Qry = mysql_query($tmp3Sql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
			while ($tmp3Row = mysql_fetch_array($tmp3Qry)) {
				
				$mySql = "INSERT INTO pengembalian_detail (no_pengembalian, kd_barang, jumlah, keterangan)
								VALUES ('$noNota', 
										'$tmp3Row[kd_barang]', 
										'$tmp3Row[jumlah]', 
										'$tmp3Row[keterangan]')";
			...
			mysql_query("DELETE FROM tmp_pengembalian WHERE kd_user='".$_SESSION['SES_LOGIN']."'", $koneksidb) 
					or die ("Gagal kosongkan tmp".mysql_error());
			
			
			echo "<script>";
			echo "window.open('source/transaksi_pengembalian_cetak.php?noNota=$noNota', width=330,height=330,left=100, top=25)";
			echo "</script>";
		}
		else{
...
$nomorTransaksi = buatKode("pengembalian", "BB");
...
?>
<div class='hero-unit'><h2 align="left">Transaksi Pengembalian Dari Suppiler</h2>
</div>
...
<table width="600" cellspacing="1" class="" style="margin-top:0px;">
	...
	<tr>
	  <td width="%"><b>No. Pengembalian </b></td>
	  <td width="1%"><b>:</b></td>
	  <td width="">
...
<table class="table table-bordered" width="800" border="0" cellspacing="1" cellpadding="2">
  ...
  <?php
	//  tabel menu 
	$tmpSql ="SELECT tmp_pengembalian.*, barang.*
			  FROM tmp_pengembalian, barang 
			  WHERE tmp_pengembalian.kd_barang=barang.kd_barang AND tmp_pengembalian.kd_user='".$_SESSION['SES_LOGIN']."' ORDER BY id";
	$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
	$nomor=0; $qtyItem = 0;
  ...
  <tr >
    ...
    <td align="center" ><a href="?page=Deletepengembalian&amp;id=<?php echo $id; ?>" target="_self" alt="Delete" onclick="pengembaliann confirm('Anda yakin menghapus <?php echo  $tmpRow['nm_panjang']; ?> ... ?')"><i class="icon-trash"></i></a></td>
  </tr>
  <?php } ?>
  <tr>
    <td>&nbsp;</td>
